...
def sarcasm(file_path, max_features, solver):
    ...
    print("Training Accuracy for Logistic Regression without stopwords : ", pipeline.score(train_texts, y_train) * 100)
    prediction = pipeline.predict(valid_texts)
    print("Test Accuracy for Logistic Regression without stopwords : ", accuracy_score(y_valid, prediction) * 100)
