...
def sarcasm(file_path, max_features):
    ...
    print("Training Accuracy for MNB with stopwords : ", pipeline.score(train_texts, y_train) * 100)
    prediction = pipeline.predict(valid_texts)
    print("Test Accuracy for MNB with stopwords : ", accuracy_score(y_valid, prediction) * 100)
